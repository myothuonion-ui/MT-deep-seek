#!/usr/bin/env python3
"""Apply the MT security hardening layer to pinned KMN-CyberSeek v2.3.3.

This script is intentionally fail-closed for critical replacements: if the pinned
upstream changes and an expected critical block can no longer be found, bootstrap
stops rather than publishing a partially-hardened tree.
"""
...
def regex_once(path: str, pattern: str, repl: str, *, required: bool = True, flags: int = 0) -> None:
    text = read(path)
...
# ---------------------------------------------------------------------------
# P0: operator intent, command-policy boundary, prompt schema, secret quoting
# ---------------------------------------------------------------------------
replace_once(
    "frontend.py",
    ...
    '''                auto_approval = st.checkbox(\n                    "Full autonomy (all risk levels)",\n                    value=False,\n                    help=("Explicitly allow autonomous execution across all risk levels. "\n                          "Safety allowlists and verifier gates still apply. Use only on "\n                          "isolated targets you own or are authorized to test.")\n                )''',
)

replace_once("ai/connector.py", "from typing import Dict, List, Optional, Any", "from typing import Dict, List, Optional, Any, Literal")
replace_once(
    "ai/connector.py",
...
# Scope becomes deny-by-default unless the operator explicitly opts out.
replace_once(
    "core/validators.py",
    ...
    '''    Scope is deny-by-default. If allowlist_str is empty/unset, targets are\n    rejected unless ALLOW_UNSCOPED_TARGETS=true is explicitly configured. This\n    prevents an accidental blank allowlist from becoming unrestricted access.''',
)
replace_once("core/validators.py", '        return True  # No allowlist configured -> unrestricted', '        return os.getenv("ALLOW_UNSCOPED_TARGETS", "false").lower() == "true"')
replace_once("core/validators.py", '        return True\n\n    try:\n        target_ip', '        return os.getenv("ALLOW_UNSCOPED_TARGETS", "false").lower() == "true"\n\n    try:\n        target_ip')
replace_once(
    "core/validators.py",
    '_ENV_ASSIGNMENT_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*=")',
    '''_ENV_ASSIGNMENT_RE = re.compile(r"^[A-Za-z_][A-Za-z0-9_]*=")\n\n# General-purpose interpreters turn a binary allowlist into an arbitrary-code\n# primitive. Autonomous execution routes these to human review instead.\nAUTO_DENIED_BINARIES = {\n    "bash", "sh", "zsh", "dash", "fish", "python", "python2", "python3",\n    "perl", "ruby", "php", "node", "nodejs", "powershell", "pwsh",\n}''',
)
replace_once(
    "core/validators.py",
    ...
    '''    FULL_AUTO_MODE bypasses manual approval, but never bypasses this structural\n    allowlist. Human-reviewed commands remain a separate trust boundary.\n\n    NOT applied to commands a human explicitly typed or clicked "approve" on —''',
)
replace_once(
    "core/validators.py",
...
# Orchestrator persistence permissions and high-risk policy setting.
replace_once("core/orchestrator.py", '        self.db_path = "kmn_cyberseek.db"', '        self.db_path = os.getenv("DB_PATH", "kmn_cyberseek.db")')
replace_once(
...
# Never interpolate captured credentials into shell syntax without quoting.
replace_once("core/orchestrator.py", '            command += f" -U \'{user}%{passwd}\'"', '            command += f" -U {shlex.quote(f\'{user}%{passwd}\')}"')
replace_once("core/orchestrator.py", '                command = re.sub(r\'\'\'-U\\s+["\']["\']\'\'\', f"-U \'{user}%{passwd}\'", command)', '                command = re.sub(r\'\'\'-U\\s+["\']["\']\'\'\', lambda _m: f"-U {shlex.quote(f\'{user}%{passwd}\')}", command)')
replace_once("core/orchestrator.py", '                command += f" -U \'{user}%{passwd}\'"', '                command += f" -U {shlex.quote(f\'{user}%{passwd}\')}"')
replace_once("core/orchestrator.py", '                    lambda m: f"{user}:{passwd}@{m.group(0)}",', '                    lambda m: shlex.quote(f"{user}:{passwd}@{m.group(0)}"),')
# Do not inject raw secrets into LLM memory; deterministic credential reuse keeps them local.
replace_once(
    "core/orchestrator.py",
...
# Initial AI command must pass the same allowlist/verifier boundary as subsequent turns.
regex_once(
    "core/orchestrator.py",
...
# Reports mask secrets by default and use owner-only file permissions.
replace_once(
...
# Never kill arbitrary processes just because they occupy a preferred port.
text = read("start.sh")
text = re.sub(r'_try_kill_port "\$BACKEND_PORT"\n', '', text)
text = re.sub(r'_try_kill_port "\$FRONTEND_PORT"\n', '', text)
text = re.sub(r'_try_kill_port "\$DOCS_PORT"\n', '', text)
text = text.replace('    echo "❌ Python3 is not installed. Please install Python 3.8+ first."', '    echo "❌ Python3 is not installed. Please install Python 3.10+ first."')
text = text.replace('    cp .env.example .env 2>/dev/null || touch .env', '    cp .env.example .env 2>/dev/null || touch .env\n    chmod 600 .env 2>/dev/null || true')
write("start.sh", text)

# Environment docs: safe defaults and explicit secret-export opt-in.
...
text = text.replace(
    ...
    '# When true, the AI may execute across risk levels without routine approval,\n# but target scope, autonomous binary/interpreter controls and verifier gates remain enforced.'
)
...
# Align old tests with hardened deny-by-default/autonomous-interpreter policy.
replace_once(
...
# Minimal project/tooling metadata for reproducible CI.
write(
    "pyproject.toml",
    '''[tool.pytest.ini_options]\nasyncio_mode = "auto"\ntestpaths = ["tests"]\n\n[tool.ruff]\ntarget-version = "py310"\nline-length = 120\n'''
)

# Version + attribution docs.
replace_once("_version.py", '__version__ = "2.3.3"', '__version__ = "2.4.0-hardened.1"')
write(
    "SECURITY_HARDENING.md",
    '''# MT Security Hardening\n\nBaseline: KMN-CyberSeek v2.3.3 at commit `3e8b08a36c6af989f30c1d564f1a1c00579dbf43`.\n\nKey changes: deny-by-default target scope; full-auto no longer bypasses autonomous allowlists; general-purpose interpreters require human review; initial AI commands pass the same allowlist/verifier gate as later commands; strict AI response enums/ranges; explicit local model selection cannot be overridden by a stale API key; session memory is marked untrusted; captured credentials are shell-quoted and withheld from LLM memory; threat-intel fetches reject private/link-local/metadata destinations and validate redirects; brute-force subprocesses use argv execution with bounded process-group cleanup; report secrets are masked by default; database/report/.env permissions are tightened; Metasploit handler commands are serialized; startup no longer kills unrelated processes occupying preferred ports.\n\nThis remains a dual-use authorized security-testing tool. Use only on systems you own or have explicit permission to test.\n'''
)
